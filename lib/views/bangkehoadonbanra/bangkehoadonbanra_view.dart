...
import 'bankehoadonbanra_function.dart';

class BangKeHoaDonBanRaView extends ConsumerStatefulWidget {
  static const name = "Bảng kê hóa đơn bán ra";
  static const title = "Bảng kê hóa đơn hàng hóa dịch vụ bán ra";

  static void show(BuildContext context) {
    showCustomDialog(context, title: title.toUpperCase(), width: 1220, height: 600, child: BangKeHoaDonBanRaView());
  }

  const BangKeHoaDonBanRaView({super.key});

  @override
  ConsumerState createState() => _BanKeHoaDonBanRaViewState();
}

class _BanKeHoaDonBanRaViewState extends ConsumerState<BangKeHoaDonBanRaView> {
  final lstThang = [for (int i = 1; i <= 12; i++) i];
  final txtNam = TextEditingController(text: DateTime.now().year.toString());
  final fc = BangKeHoaDonBanRaFunction();
    ...
    return Scaffold(
      ...
      child: DataGrid(
        ...
        columns: [
          ...
          DataGridColumn(title: ['Số HD', 'SoHD'], width: 80, cellColor: CellColor.red),
          ...
          DataGridColumn(title: ['Tiền chưa VAT', 'CongTien'], width: 125, columnType: ColumnType.num),
          DataGridColumn(title: ['Tiền thuế', 'TienThue'], width: 125, columnType: ColumnType.num),
