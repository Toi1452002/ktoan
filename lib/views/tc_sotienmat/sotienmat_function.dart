...
class SoTienMatFunction {
  Future<List<Map<String, dynamic>>> get({DateTime? tN, DateTime? dN}) async {
    return await SoTienRepository().getSoTienMat(tN: Helper.yMd(tN), dN: Helper.yMd(dN));
  }

  Future<List<String>> getTon({DateTime? tN, DateTime? dN}) async {
    final data = await SoTienRepository().getTonSoTienMat(tN: Helper.yMd(tN), dN: Helper.yMd(dN));
