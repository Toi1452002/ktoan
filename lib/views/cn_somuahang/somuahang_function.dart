...
class SoMuaHangFunction {
  Future<List<Map<String, dynamic>>> loadNC() async {
  ...
  Future<void> loadData(TrinaGridStateManager state, DateTime tN, DateTime dN, String maKhach) async {
    final data = await CongNoRepository().getSoMuaHang(tN: Helper.yMd(tN), dN: Helper.yMd(dN), maKhach: maKhach);
    state.removeAllRows();
    state.appendRows(
      data.map((e) {
        return TrinaRow(
